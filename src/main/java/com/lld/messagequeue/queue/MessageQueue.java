...
import com.lld.messagequeue.workers.SubscriberWorker;
  ...
  public boolean enqueue(Message message) {
    ...
    try {
      while (size() == maxCapacity && !Thread.currentThread().isInterrupted()) {
        try {
//          System.out.println("Message Queue is full, bellow message with message id %s is waiting to be enqueued %s" + message.getId());
          logger.warn("Message Queue is full, bellow message with message id %s is waiting to be enqueued", message.getId());
